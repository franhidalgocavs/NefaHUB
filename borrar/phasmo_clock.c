/*
 * PhasmoClock — Hunt & Incense Timer (C / Win32 API)
 * ───────────────────────────────────────────────────
 * Compile:
 *   gcc -O2 -mwindows -o phasmo_clock.exe phasmo_clock.c -lgdi32 -luser32 -lwinmm
 *
...
/* ── Titles ───────────────────────────────────────────────── */
#define MAIN_TITLE    L"PhasmoClock"
#define OVERLAY_TITLE L"PhasmoClockOV"
#define CONFIG_FILE   "phasmo_config.json"
...
static void open_settings(void) {
    ...
    if (!registered) {
        ...
        wc.lpszClassName = L"PhasmoSettings";
...
int WINAPI WinMain(HINSTANCE hInst, HINSTANCE hPrev, LPSTR cmdLine, int cmdShow) {
    ...
    wc.lpszClassName = L"PhasmoClockMain";
    wc.hCursor       = LoadCursor(NULL, IDC_ARROW);
    RegisterClassExW(&wc);
    ...
    wc2.lpszClassName = L"PhasmoClockOverlay";
