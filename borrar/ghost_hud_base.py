...
# ── Color palette (Standard Dark Theme) ───────────────────────
...
OV_W, OV_H = 420, 280  # Geometría del overlay
...
# ── Font tokens (Segoe UI estándar del sistema) ────────────────
F_CLOCK_MAIN  = ("Segoe UI", 36, "bold")   # reloj cacería principal
F_CLOCK_OV    = ("Segoe UI", 30, "bold")   # reloj cacería en el overlay
F_STATUS_MAIN = ("Segoe UI", 14, "bold")   # estado cacería principal
F_STATUS_OV   = ("Segoe UI", 9, "bold")    # estado cacería en el overlay
F_INCENSE     = ("Segoe UI", 16, "bold")   # incienso principal
F_INCENSE_OV  = ("Segoe UI", 30, "bold")   # incienso en el overlay
F_INCENSE_ICO = ("Segoe UI", 24)            # emoji de incienso en el overlay
...
# ═════════════════════════════════════════════════════════════
    ...
    def _toggle_cursed(self):
        ...
        if self.hunt_active:
            if self.is_cursed:
                ...
                self.hunt_time -= self.cursed_mod
            self._recalc()
            self._set_status("CACERIA ACTIVA", C_RED)
    ...
    def _recalc(self):
        ...
        self.lbl_curse_val.config(
            text=f"Maldita (+{int(self.cursed_mod)}s)" if self.is_cursed else "Normal",
            fg=C_RED if self.is_cursed else C_DIM)
    ...
    def _start_incense(self):
        if self.incense_active:
            ...
            if self.incense_paused or getattr(self, "incense_was_paused", False):
                ...
                col = C_ACCENT
                self.lbl_incense_status.config(text="Incienso corriendo", fg=col)
    ...
    def _reset_incense(self):
        ...
        self.lbl_incense_status.config(text="Incienso listo", fg=C_DIM)
        self._render_incense("00:00.0", C_DIM)
    ...
    def _render_clock(self, t, state="idle", total=None):
        ...
        if state == "idle":
            ...
            fg = C_PURPLE if self.is_cursed else C_RED
        elif state == "cooldown":
        ...
        if state == "hunt":
            ...
            col = C_PURPLE if self.is_cursed else C_RED
            ...
            self.canvas_bar.coords(self.bar_rect, 0, 0, int(w * pct), 8)
            self.canvas_bar.itemconfig(self.bar_rect, fill="#d0d0d0")
    ...
    def _build_main_ui(self):
        ...
        tk.Label(self.cfg_panel, text="Ajustes",
                 font=("Segoe UI", 9, "bold"), bg=C_PANEL, fg=C_DIM).pack(pady=(8,6))

        self._cfg_row("Mapa:", "lbl_map_val",   "#ffffff")
        self._cfg_row("Modo:", "lbl_curse_val", C_BLUE)
        self._cfg_row("Dif:",  "lbl_hunt_val",  "#ffffff")

        tk.Frame(self.cfg_panel, bg="#444444", height=1).pack(fill="x", padx=8, pady=8)

        self.lbl_total = tk.Label(self.cfg_panel, text="TOTAL: --s",
                                  font=("Segoe UI", 9, "bold"), bg=C_PANEL, fg="#ffffff")
        self.lbl_total.pack(pady=2)

        tk.Frame(self.cfg_panel, bg="#444444", height=1).pack(fill="x", padx=8, pady=8)

        self.lbl_hotkey_hint = tk.Label(self.cfg_panel,
                 text="", font=("Segoe UI", 8), bg=C_PANEL, fg="#aaaaaa", justify="left")
        self.lbl_hotkey_hint.pack(padx=8, pady=4)
        self._refresh_hotkey_hint()

        tk.Button(self.cfg_panel, text="Opciones",
                  font=("Segoe UI", 8, "bold"), bg="#3e3e3e", fg="#ffffff",
        ...
        self.lbl_incense_status = tk.Label(inc_row, text="Incienso",
        ...
        self.lbl_status = tk.Label(top_row, text="",
                                   font=F_STATUS_MAIN, bg=C_BLOCK, fg=C_GREEN)
        ...
        self.lbl_hunt_cfg = tk.Label(clk_inner, text="[--]", font=("Segoe UI", 12, "bold"), bg=C_BLOCK, fg=C_DIM)
        self.lbl_hunt_cfg.pack(side="left", padx=(0, 6), anchor="s", pady=(0,8))

        self.lbl_hunt_clock = tk.Label(clk_inner, text="00:00.0",
                                       font=F_CLOCK_MAIN, bg=C_BLOCK, fg=C_DIM)
        ...
        self.bar_rect = self.canvas_bar.create_rectangle(0, 0, 0, 8, fill="#d0d0d0", width=0)
    ...
    def _open_settings(self):
        ...
        win.title("Opciones — NefaHUB Base")
        ...
        # ── Title ─────────────────────────────────────────────
        tk.Label(win, text="Opciones", font=("Segoe UI", 12, "bold"),
                 bg=C_PANEL, fg="#ffffff").pack(pady=(14, 6))
        tk.Frame(win, bg="#444444", height=1).pack(fill="x", padx=16)

        # ── DEFAULTS ──────────────────────────────────────────
        tk.Label(win, text="Valores por defecto",
                 font=("Segoe UI", 8, "bold"), bg=C_PANEL, fg=C_DIM
        ...
        tk.Label(map_row, text="Mapa:", font=("Segoe UI", 9), bg=C_BLOCK,
                 fg="#ffffff", width=12, anchor="w").pack(side="left")
        ...
        for i, lbl in enumerate(MAP_LABELS.values()):
            tk.Radiobutton(map_row, text=lbl, variable=map_var, value=i,
                           font=("Segoe UI", 9), bg=C_BLOCK, fg="#ffffff",
        ...
        tk.Label(hunt_row, text="Dificultad:", font=("Segoe UI", 9), bg=C_BLOCK,
                 fg="#ffffff", width=12, anchor="w").pack(side="left")
        ...
        tk.Frame(win, bg="#444444", height=1).pack(fill="x", padx=16, pady=(10, 0))
        ...
        tk.Label(header_row, text="Dificultad", font=("Segoe UI", 8, "bold"), bg=C_BLOCK, fg=C_DIM, width=12, anchor="w").pack(side="left")
        tk.Label(header_row, text="CHICO", font=("Segoe UI", 8, "bold"), bg=C_BLOCK, fg=C_DIM, width=8, anchor="center").pack(side="left", padx=6)
        tk.Label(header_row, text="MEDIANO", font=("Segoe UI", 8, "bold"), bg=C_BLOCK, fg=C_DIM, width=8, anchor="center").pack(side="left", padx=6)
        tk.Label(header_row, text="GRANDE", font=("Segoe UI", 8, "bold"), bg=C_BLOCK, fg=C_DIM, width=8, anchor="center").pack(side="left", padx=6)
        ...
        for hk in HUNT_CYCLE:
            ...
            tk.Label(row_frame, text=HUNT_LABELS[hk], font=("Segoe UI", 9), bg=C_BLOCK, fg="#ffffff", width=12, anchor="w").pack(side="left")
            ...
            for mk in MAP_CYCLE:
                ...
                entry = tk.Entry(row_frame, textvariable=var, font=("Segoe UI", 9, "bold"), bg=C_ROOT, fg="#ffffff",
                                 insertbackground="#ffffff", width=8, justify="center", relief="sunken", bd=1)
                entry.pack(side="left", padx=6)

        # Cursed modifier row
        cursed_row = tk.Frame(times_frame, bg=C_BLOCK)
        cursed_row.pack(fill="x", pady=(8, 2))
        tk.Label(cursed_row, text="Mod. Maldita:", font=("Segoe UI", 9), bg=C_BLOCK, fg="#ffffff", width=12, anchor="w").pack(side="left")
        cursed_var = tk.StringVar(value=str(int(self.cursed_mod) if self.cursed_mod.is_integer() else self.cursed_mod))
        cursed_entry = tk.Entry(cursed_row, textvariable=cursed_var, font=("Segoe UI", 9, "bold"), bg=C_ROOT, fg="#ffffff",
                                insertbackground="#ffffff", width=8, justify="center", relief="sunken", bd=1)
        cursed_entry.pack(side="left", padx=6)
        tk.Label(cursed_row, text="segundos extra", font=("Segoe UI", 8), bg=C_BLOCK, fg=C_DIM).pack(side="left", padx=6)
        ...
        tk.Button(reset_row, text="Restablecer tiempos por defecto",
                  font=("Segoe UI", 8, "bold"), bg="#3e3e3e", fg="#ffffff",
                  relief="raised", bd=1, cursor="hand2", padx=6, pady=4,
                  activebackground="#4e4e4e", activeforeground="#ffffff",
        ...
        for action, label in ACTION_LABELS.items():
            ...
            tk.Label(row, text=label, font=("Segoe UI", 8), bg=C_BLOCK,
        ...
        tk.Button(bf, text="  GUARDAR  ", font=("Segoe UI", 9, "bold"),
        ...
        tk.Button(bf, text="  CANCELAR  ", font=("Segoe UI", 9, "bold"),
                  bg="#3e3e3e", fg="#ffffff", relief="raised", bd=1, padx=8, pady=4,
                  activebackground="#4e4e4e", cursor="hand2",
    ...
    def _cfg_row(self, label_text, attr, fg):
        ...
        tk.Label(row, text=label_text, font=F_CONFIG_LBL,
                 bg=C_PANEL, fg="#aaaaaa", width=10, anchor="w").pack(side="left")
    ...
    def _build_overlay(self, wx, wy):
        ...
        self.ov_status = tk.Label(ov, text="",
                                  font=F_STATUS_OV, bg=C_CHROMA, fg=C_GREEN)
        ...
        self.ov_cfg_hint = tk.Label(self.ov_clock_row, text="[--]", font=("Segoe UI", 12, "bold"), bg=C_CHROMA, fg=C_DIM)
        self.ov_cfg_hint.pack(side="left", anchor="s", pady=(0,5), padx=(0,4))

        self.ov_clock = tk.Label(self.ov_clock_row, text="00:00.0",
                                 font=F_CLOCK_OV, bg=C_CHROMA, fg=C_DIM)
    ...
    # ══════════════════════════════════════════════════════════
    # VIEW MODE CYCLING (F9) — Alternar modo Normal y Overlay Transparente
    # ══════════════════════════════════════════════════════════
